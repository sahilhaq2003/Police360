      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center space-x-6">
              ...
              <div className="flex-1">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm"><div><span className="font-semibold text-gray-600">Suspect ID:</span><span className="ml-2 font-mono">#{suspect.suspectId||'N/A'}</span></div><div><span className="font-semibold text-gray-600">NIC:</span><span className="ml-2">{suspect.nic||'N/A'}</span></div></div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-lg p-6"><h2 className="text-xl font-bold text-[#0B214A] mb-4 border-b border-gray-200 pb-2">Additional Information</h2><div className="space-y-4">{suspect.otherInfo && (<div><span className="font-semibold text-gray-600">Other Information:</span><p className="text-gray-800 mt-1 whitespace-pre-wrap">{suspect.otherInfo}</p></div>)}{suspect.crimeInfo && (<div><span className="font-semibold text-gray-600">Crime Information:</span><p className="text-gray-800 mt-1 whitespace-pre-wrap">{suspect.crimeInfo}</p></div>)}</div></div>
          </div>

          <div className="space-y-6">
            {suspect.fingerprints && suspect.fingerprints.length>0 && (
              <div className="bg-white rounded-lg shadow-lg p-6"><h2 className="text-xl font-bold text-[#0B214A] mb-4 border-b border-gray-200 pb-2">Fingerprints</h2><div className="grid grid-cols-2 gap-3">{suspect.fingerprints.map((p,i)=> (<div key={i} className="border border-gray-300 rounded p-2 text-center"><div className="h-20 bg-gray-100 rounded mb-2 flex items-center justify-center overflow-hidden">{p?.url ? (<img src={getMediaUrl(p.url)} alt={`fp-${i+1}`} className="w-full h-full object-cover"/>) : (<span className="text-xs text-gray-500">Print #{i+1}</span>)}</div><p className="text-xs text-gray-600 truncate">{p?.name||p?.url||`#${i+1}`}</p></div>))}</div><div className="mt-4 space-y-2"><button className="w-full px-4 py-2 bg-yellow-600 text-white rounded hover:bg-yellow-700 text-sm">Update Status</button><button className="w-full px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 text-sm">Add New Arrest</button><button className="w-full px-4 py-2 border border-gray-300 text-gray-700 rounded hover:bg-gray-50 text-sm">View Full History</button></div></div>
            )}
